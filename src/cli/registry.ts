...
export const COMMANDS: readonly CommandEntry[] = [
  ...
  {
    name: 'poll',
    usage: 'illuminate poll <file.html> [--follow]',
    summary: 'Wait for a dispatch; --follow stays attached, streaming.',
  },
  {
    name: 'answer',
...
export const PLAYBOOKS: readonly PlaybookEntry[] = [
  {
    ...
    body: [
      'A section cites its source in one of two forms, making two DIFFERENT',
      'claims. Pick the one that is true.',
      '',
      'PINNED -- one region, at one revision, hash-verified:',
      '  data-src="<repo-relative-path>[#Lx-Ly]"  data-rev="<commit-sha>"',
      '  data-anchor-hash="<hash of the anchored region>"',
      'Omit data-rev to resolve against HEAD (marked unpinned).',
      'The hash is normalized for line endings and trailing whitespace only,',
      'which is what makes reformatting safe. It is also the ONLY thing that',
      'makes drift detection possible: a pinned anchor is the only form that',
      'can tell you the code moved or changed under the prose.',
      '',
      'FILE-LEVEL -- a path, or a list of them, with NO hash:',
      '  data-src="<repo-relative-path>"',
      '  data-files="<path>, <path>, ..."',
      'Served from the working tree and labelled as such on the wire, so a',
      'working-tree read is never mistaken for a revision-pinned one. Each',
      'listed file resolves on its own, so an answer can say which one it',
      'could not read. Use this when the claim is about whole files, or when',
      'stamping a hash per file is ceremony you would rather skip -- that',
      'ceremony is why ordinary artifacts used to end up ungrounded.',
  ...
  {
    id: 'attach',
    title: 'Staying attached to a session',
    body: [
      'A harness should attach ONCE and keep receiving work, rather than',
      're-invoking a one-shot poll in a loop:',
      '  illuminate poll <file.html> --follow',
      'It stays attached, printing ONE JSON envelope per line (NDJSON) as',
      'each interaction is queued -- review, verification or requested',
      'change alike. It is silent while idle, so no output means nothing',
      'is waiting, never that the connection dropped. Two things change',
      'that: when the session has ended it prints one stderr line and',
      'exits 0 (the review is over, not the work), and while the browser',
      'tab is closed it backs off (1s doubling to 8s) and stays attached,',
      'so a closed tab never needs a restart.',
      'Run a DEDICATED agent per envelope, with the parameters the envelope',
      'already carries -- never parameters you chose yourself:',
      '  role         which of tutor/verifier/researcher/author/implementer',
      '  model_tier   which tier to run that agent at',
      '  tools        the exact tool list it is allowed; [] means zero tools',
      '  deadline_ms  how long it may take',
      'These come from illuminate\'s own routing policy. Re-deriving them',
      ...
      '<port> --model <name> --tier <haiku|sonnet|opus> --stdin; the',
      'envelope names the command in return_to, with the model and tier',
